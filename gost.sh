...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/xtfneeq1202/Gost-Tunnel/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/xtfneeq1202/Gost-Tunnel/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
update_sh() {
  ol_version=$(curl -L -s --connect-timeout 5 https://raw.githubusercontent.com/xtfneeq1202/Gost-Tunnel/master/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [ -n "$ol_version" ]; then
    if [[ "$shell_version" != "$ol_version" ]]; then
      ...
      [yY][eE][sS] | [yY])
        wget -N --no-check-certificate https://raw.githubusercontent.com/xtfneeq1202/Gost-Tunnel/master/gost.sh
        echo -e "更新完成"
        exit 0
        ;;
      *) ;;
